...
class Executor:
    ...
    def __init__(self):
        if digest_files.fileSetupComplete():
            ...
            self.mapper = FileMapper(preload=True)
    ...
    def validate(self,tar, tar_id, mode, ref, ref_id, enriched, out_dir, runs, background_model, replace):
        ...
        single_validation(tar=tar, tar_id=tar_id,mode=mode, ref=ref, ref_id=ref_id,enriched=enriched, out_dir=out_dir,
                          runs=runs,background_model=background_model, replace=replace, mapper=self.mapper)
    ...
    def run_set(self,data):
        ...
        return self.validate(tar=data["target"], tar_id=data["target_id"], mode="set", out_dir=data["out"], runs=data["runs"],
    ...
    def run_cluster(self,data):
        ...
        return self.validate(tar=data["target"], tar_id=data["target_id"], mode="cluster", out_dir=data["out"], runs=data["runs"],
                        replace=data["replace"],ref=None, ref_id=None, enriched=None, background_model=None)

    def run_set_set(self,data):
        print("Executing set-set validation with uid: " + str(data["uid"]))
        return self.validate(tar=data["target"], tar_id=data["target_id"], ref_id=data["reference_id"], ref=data["reference"], mode="set-set", out_dir=data["out"], runs=data["runs"],
                        replace=data["replace"], enriched=data["enriched"], background_model=None)

    def run_id_set(self,data):
        print("Executing id-set validation with uid: " + str(data["uid"]))
        return self.validate(tar=data["target"], tar_id=data["target_id"], ref_id=data["reference_id"], ref=data["reference"], mode="id-set", out_dir=data["out"], runs=data["runs"],
                        replace=data["replace"], enriched=data["enriched"], background_model=None)
