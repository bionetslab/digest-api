...
def run_task(uid, mode, parameters, set_files):
    ...
    def set_result(results):
        ...
        if t.sc:
            ...
            if "sigContTarget" in parameters and parameters["sigContTarget"] is not None:
                targets = parameters["sigContTarget"]
                if mode == 'cluster':
                    targets = pd.DataFrame(targets, columns=["id"])
            t.sc_status = json.dumps({"done": 0, "total": len(targets)})
            dispatch_sig_contr_calculation(uid=uid, tar=targets)
            t.save()
