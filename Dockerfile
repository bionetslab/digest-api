...
RUN conda install -c conda-forge -y django=4.0.2 graph-tool
...
RUN pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple biodigest==0.0.20
COPY . /usr/src/digest/
